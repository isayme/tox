...
|--------|---------|-----------|
| `grpc` / `grpcs` | `tunnel/grpc` | gRPC bidirectional stream + JWT auth |
...
**Authentication** — tunnel password is hashed with PBKDF2 (`util/kdf.go`) and verified per-protocol: gRPC uses per-RPC metadata token; WebSocket and HTTP/2/3 use an HTTP `token` header. gRPC additionally has JWT support (`util/jwt.go`).
